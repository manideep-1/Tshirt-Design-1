        ...
        <div className="row">
          {OrderDetails.map((order_info, index) => (
            <div className="col-12">
              ...
              <div
                class={`${style.addbordertoorder}  ${style.orderscardcolor}`}
              >
                <img
                  alt=""
                  class={style.tshirtorder}
                ...
                <br></br>
                {order_info.Orderdone ? (
                  <label class={style.labeldelivery}>delivered</label>
                ) : (
                  <label class={style.labeldeliveryfailed}>order failed</label>
                )}
                <Link
                  to="/accounts/orders/ordersinfo"
                  style={{ textDecoration: "none", color: "inherit" }}
                >
                  <button
                    className="btn btn-outline-primary"
                    id={style.buttonorderinfo}
                  >
                    ORDER INFO
                  </button>
                </Link>
